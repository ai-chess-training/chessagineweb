...
export class Board {
  ...
  private checkDirectionForPin(x: number, y: number, dx: number, dy: number, attackingColor: PieceColour): 
    ...
    let firstPiece: {piece: PieceType, color: PieceColour, x: number, y: number} | null = null;
    let secondPiece: {piece: PieceType, color: PieceColour, x: number, y: number} | null = null;
  ...
  private calculateFork(piece: PieceSymbol, pieceSq: Square, side: Color, chess: Chess): string[] {
    const forks: string[] = [];
    ...
    const attackableTargets: {square: Square, piece: PieceSymbol, isDeadly: boolean}[] = [];
  ...
  private getAttackedSquares(piece: PieceSymbol, square: Square, chess: Chess): Square[] {
    ...
    switch (piece) {
      case 'p': // pawn
        ...
        if (newRank >= 0 && newRank <= 7) {
          if (file > 0) {
            attackedSquares.push(`${String.fromCharCode('a'.charCodeAt(0) + file - 1)}${newRank + 1}` as Square);
          }
          if (file < 7) {
            attackedSquares.push(`${String.fromCharCode('a'.charCodeAt(0) + file + 1)}${newRank + 1}` as Square);
          }
      ...
      case 'n': // knight
        ...
        for (const [df, dr] of knightMoves) {
          ...
          if (newFile >= 0 && newFile <= 7 && newRank >= 0 && newRank <= 7) {
            attackedSquares.push(`${String.fromCharCode('a'.charCodeAt(0) + newFile)}${newRank + 1}` as Square);
          }
        }
        break;
  ...
  private addSlidingAttacks(attackedSquares: Square[], file: number, rank: number, directions: number[][], chess: Chess): void {
    for (const [df, dr] of directions) {
      ...
      while (true) {
        ...
        const targetSquare = `${String.fromCharCode('a'.charCodeAt(0) + currentFile)}${currentRank + 1}` as Square;
